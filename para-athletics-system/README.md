...
- [x] **管理ダッシュボード**（`/dashboard`）
      — レディネス推移・HRV日次＋7日移動平均・睡眠スコア・HRV変動係数(CV)の信号色表示
      （依存ライブラリ不要の軽量SVGチャート／デモデータ生成つき）
- [x] **スケジュール・移動**（`/schedule`）と、ダッシュボードでの
      **移動×HRVクロス可視化**（移動・試合マーカーを HRV トレンドに重畳）
- [x] **トレーニング記録**（`/training`）と、ダッシュボードの
      **トレーニング負荷**（セッションRPE法＝時間×RPE）の可視化
- [x] **財務・経費**（`/expenses`）— カテゴリ別の記録・集計
- [ ] 認証（Supabase Auth）と athlete_members によるロール解決
- [ ] レシート画像の AI-OCR 自動取り込み（財務）
- [ ] スケジュールの Supabase 同期（現状はオフライン保存のみ）
- [ ] ダッシュボードのドリルダウン・期間フィルタ（日付ピッカー）
- [ ] トレーニング負荷・Optimize Score 本実装
